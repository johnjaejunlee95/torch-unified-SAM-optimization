...
class Bottleneck(nn.Module):
    ...
    def forward(self, x):
        ...
        if residual_channel != shortcut_channel:
            padding = torch.zeros(batch_size, residual_channel - shortcut_channel, *featuremap_size, requires_grad=True).to(out.device)
            out += torch.cat((shortcut, padding), 1)
        else:
            out += shortcut 
...
class PyramidNet(nn.Module):
    def __init__(self, depth, alpha, num_classes, bottleneck=True, dataset='cifar'):
