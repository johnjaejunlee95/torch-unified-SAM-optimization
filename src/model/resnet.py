...
class ResNet(nn.Module):
    def __init__(self, block, num_blocks, dropout_rate, num_classes=100):
        ...
        self.gap = nn.Sequential(nn.AdaptiveAvgPool2d((1, 1)), 
                                 nn.Flatten())
        if dropout_rate > 0.:
            self.dropout = nn.Dropout(dropout_rate)  # Default dropout rate, can be modified
    ...
    def forward(self, x, is_feature=False):
        x_proj = F.relu(self.bn1(self.conv1(x)))
        out = self.layer1(x_proj)  # shape: [batch_size, 64, 32, 32]
        out = self.layer2(out) # shape: [batch_size, 128, 16, 16]
        out = self.layer3(out) # shape: [batch_size, 256, 8, 8]
        out = self.layer4(out)
        features = self.gap(out)
        if hasattr(self, 'dropout'):
            features = self.dropout(features)
        logits = self.linear(features)
        
        if is_feature:
            return logits, x_proj, features
